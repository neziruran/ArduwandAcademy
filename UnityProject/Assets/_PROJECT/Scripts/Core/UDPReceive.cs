...
public class UDPReceiver : MonoBehaviour
{
    ...
    private const float INITIAL_WAIT_TIME = 2f;
    private float _initialWaitTimer;
    private DateTime _lastPacketTime;
    private volatile string _currentGesture = "Unknown";
    private volatile float _currentConfidence = 0f;
    ...
    {
        return isConnected ? _currentGesture == "Unknown" ? "Unknown" : _currentGesture : "Unknown";
    }
    ...
    private void Update()
    {
        ...
        if (enableDebugPanel)
        {
            ...
            {
                if (gestureText != null)
                    gestureText.SetText($"Gesture: {_currentGesture ?? "None"}");
